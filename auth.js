...
const API_BASE_URL = (window.API_CONFIG && window.API_CONFIG.API_BASE_URL) || '../backend/api'; // Base URL for API endpoints (from frontend/config.js)

function apiEndpoint(nameOrPath) {
    if (window.API_CONFIG && window.API_CONFIG.ENDPOINTS && window.API_CONFIG.ENDPOINTS[nameOrPath]) {
        return window.API_CONFIG.ENDPOINTS[nameOrPath];
    }
    return nameOrPath; // assume caller passed full filename (e.g. 'login.php')
}
...
async function apiCall(endpoint, method = 'POST', data = null) {
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (!response.ok) {
                const msg = json.message || response.statusText || `HTTP ${response.status}`;
                throw new Error(msg);
            }
            return json;
        }
        ...
        // If 2xx but not JSON, return raw text inside an object
...
const result = await apiPost(apiEndpoint('register'), {
            full_name: fullName,
            email: email,
            phone: contactNumber,
...
const result = await apiPost(apiEndpoint('login'), {
        ...
        localStorage.setItem('cargoConnectCurrentUser', JSON.stringify({
            ...
            role: user.user_role || user.role || 'customer',
            fullName: user.full_name,
            email: user.email_address,
            role: user.user_role || user.role || 'customer',
